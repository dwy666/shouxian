<template>
  <div id="peasonSetting">
      ...
      <el-aside width="230px">
        <el-tree v-if="treeData.length != 0" :data="treeData" @node-click="handleNodeClick">
          <span class="custom-tree-node" slot-scope="{ node }">
            ...
            <span class="treeInfoIcon" @click.stop="lookInfo(node.data.type,node.data.testify,node.data.isMessage)">
              <img v-if="node.data.testify == 1" :src="noInfoIcon" alt />
              <img v-else :src="infoIcon" alt />
            </span>
            <span v-if="node.data.add" class="treeAddIcon" @click.stop="addTeam(node.data.type=='A'?'B':node.data.type=='B'?'B':'C')">
              <img :src="addIcon" alt />
            </span>
          </span>
        </el-tree>
        ...
        <div class="hasTeam" v-if="treeData.length != 0">
          ...
          <el-main>
            <div class="controlBtn clearfix">
              <el-button @click="addTeam('D')" type="warning" icon="el-icon-plus">新建</el-button>
            ...
            <div class="userForm">
              <el-table :data="tableData" style="width: 100%">
                  ...
                  <template slot-scope="scope">
                    ...
                    <span v-if="scope.row.status=='3'" @click="reAddBtn(scope.row.username,scope.row.creator)" v-clickOutSide="handleClose" style="top:15px;position:absolute;left:145px;">
                      ...
                      <div v-if="reDiv" class="reAdd">
                        ...
                        <span style="color:#f29716;margin-left:6px;cursor: pointer;" @click="addTeam('D')">重新创建</span>
                      </div>
                    </span>
                  </template>
                </el-table-column>
                ...
                <el-table-column class-name="operation" label="操作" align="center">
                  <template slot-scope="scope">
                    <span @click="updateInfo">{{scope.row.operation}}</span>
                  </template>
                </el-table-column>
              </el-table>
            </div>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <div v-if="isAdd == 'updateInfo'">
        <el-form label-width="100px" style="padding:40px 24px 122px;border:1px solid #00000006">
          <el-form-item label="子账户名称：">
            <el-input v-model="childrenName" placeholder style="width:352px;text-align:left"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <dialogComponent v-else-if="isAdd != 'deleteTeam'" :teamType="teamType" :isAdd="isAdd" :teamIsTestify="teamIsTestify" :isMessage="isMessage" />
      <div v-else style="font-size:14px;color:#00000065;border-bottom:1px solid #00000006;padding:24px 24px 66px;text-align:left">删除之后该团队下的所有团队及子账户均被删除。</div>
  ...
  export default {
    ...
    methods: {
      ...
      addTeam(type) {
        ...
        this.teamType = type;
        this.isAdd = "addTeam";
        this.dialogTitle = type == "D" ? "新增子账户" : "新增团队";
      },
      // 查询团队信息
      ...
      deleteTeam(type, label) {
        ...
        this.teamName = label;
        this.dialogTitle = "确定删除" + teamName + "？";
      },
      // 确定新增团队按钮
      addTeamBtn() {
      ...
      updateInfo(id) {
        this.dialogFormVisible = true;
        this.isAdd = "updateInfo";
        this.dialogTitle = "修改信息";
      },
      bulkImport() {
        this.dialogFormVisible = true;
        this.isAdd = "bulkImport";
