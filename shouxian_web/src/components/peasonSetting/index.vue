<template>
	<div id="peasonSetting">
				...
				<el-tree v-if="treeData.length != 0" :data="treeData" @node-click="handleNodeClick">
					<span class="custom-tree-node" slot-scope="{ node }">
						...
						<span v-if="node.data.add" class="treeAddIcon"
							@click.stop="addTeam(node.data.type=='A'?'B':node.data.type=='B'?'C':'C')">
							<img :src="addIcon" alt />
						</span>
					</span>
				</el-tree>
								...
								<el-table-column prop="status" label="状态" align="center">
									<template slot-scope="scope">
										...
										<span v-if="scope.row.status=='3'"
											...
											v-clickOutSide="handleClose" style="top:15px;position:absolute;left:145px;">
											<img :src="infoIcon" alt />
											<div v-if="reDiv" class="reAdd">
												系统已解除{{reUsername}}和{{reCreator}}的关系，若想获取该账户，请
												<span style="color:#f29716;margin-left:6px;cursor: pointer;"
													@click="addTeam('D')">重新创建</span>
											</div>
										</span>
									</template>
								</el-table-column>
		...
		<el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
			...
			<dialogComponent v-else-if="isAdd != 'deleteTeam'" :teamType="teamType" :isAdd="isAdd"
			...
			<div v-else
				...
				{{teamType == 'B'?'删除之后该团队下的所有团队及子账户均被删除。':'删除之后该团队下的所有子账户均被删除。'}}</div>
