      ...
      <el-aside width="230px">
        <el-tree v-if="treeData.length != 0" :data="treeData" @node-click="handleNodeClick">
          <span class="custom-tree-node" slot-scope="{ node }">
            ...
            <span class="treeDeleteIcon">
              <img :src="deleteIcon" alt @click.stop="deleteTeam(node.data.type,node.label)" />
            </span>
            <span class="treeInfoIcon" @click.stop="lookTeamInfo(node.data.type,node.data.testify,node.data.isMessage)">
              <img v-if="node.data.testify == 1" :src="noInfoIcon" alt />
              <img v-else :src="infoIcon" alt />
            </span>
            <span v-if="node.data.add" class="treeAddIcon" @click.stop="addTeam(node.data.type=='A'?'B':node.data.type=='B'?'C':'C')">
              <img :src="addIcon" alt />
            </span>
          </span>
        </el-tree>
        ...
        <el-button class="addHeight" type="warning" icon="el-icon-plus" plain @click="addTeam('A')">新建高级合伙人</el-button>
      </el-aside>
      <el-container>
        <div class="hasTeam" v-if="treeData.length != 0">
          ...
          <el-main>
            <div class="controlBtn clearfix">
              <el-button @click="addPeasonal()" type="warning" icon="el-icon-plus">新建</el-button>
              ...
              <el-table :data="tableData" style="width: 100%">
                ...
                <el-table-column prop="status" label="状态" align="center">
                  <template slot-scope="scope">
                    ...
                    >{{scope.row.status == 1? '待认证':scope.row.status == 2?'已开通':"认证失效"}}</span>
                    <span style>{{ scope.row.message}}</span>
                    <span v-if="scope.row.status=='3'" @click="reAddBtn(scope.row.username,scope.row.creator)" v-clickOutSide="handleClose" style="top:15px;position:absolute;left:145px;">
                      <img :src="infoIcon" alt />
                      <div v-if="reDiv" class="reAdd">
                        系统已解除{{reUsername}}和{{reCreator}}的关系，若想获取该账户，请
                        <span style="color:#f29716;margin-left:6px;cursor: pointer;" @click="addPeasonal()">重新创建</span>
                      </div>
                    </span>
                  </template>
                </el-table-column>
                ...
                <el-table-column class-name="operation" label="操作" align="center">
                  <template slot-scope>
                    <span @click="lookPeasonalTeam">子账户信息</span>
                  </template>
                </el-table-column>
              </el-table>
            </div>
            <noTeam v-if="tableData.length == 0" :data="'建立'" :type="'Peasonal'" @establishPeasonal="addPeasonal" />
            <el-pagination
        ...
        <noTeam v-if="treeData.length == 0" :data="'建立团队'" :type="'headist'" @establishTeam="addTeam('A')" />
      </el-container>
    </el-container>
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <dialogComponent v-if="type != 'deleteTeam'" :teamType="teamType" :type="type" :teamIsTestify="teamIsTestify" :isMessage="isMessage" />
      <div v-else style="font-size:14px;color:#000000A6;border-bottom:1px solid #00000006;padding:40px 24px 66px;text-align:left">{{teamType == 'B'?'删除之后该团队下的所有团队及子账户均被删除。':'删除之后该团队下的所有子账户均被删除。'}}</div>
      <div slot="footer" class="dialog-footer clearfix">
        <el-button v-if="type != 'deleteTeam'" @click="dialogFormVisible = false">取 消</el-button>
...
<script>
  export default {
    ...
    methods: {
      ...
      addPeasonal() {
        this.dialogFormVisible = true;
        this.type = "addPeasonal";
        this.dialogTitle = "新增子账户";
      },
      // 查询团队信息
      lookTeamInfo(type, testify, isMessage) {
        this.dialogFormVisible = true;
        this.type = "lookTeamInfo";
        this.teamType = type;
      ...
      deleteTeam(type, label) {
        this.dialogFormVisible = true;
        this.type = "deleteTeam";
        this.teamType = type;
        this.teamName = label;
        this.dialogTitle = "确定删除" + this.teamName + "？";
      },
      // 确定新增团队按钮
      addTeamBtn() {
        this.dialogFormVisible = false;
        if (this.teamType == "lookTeamInfo") {
        } else {
          this.$message({
      ...
      lookPeasonalTeam(id) {
        this.dialogFormVisible = true;
        this.type = "lookPeasonalTeam";
        this.dialogTitle = "修改信息";
      },
      bulkImport() {
        this.dialogFormVisible = true;
        this.type = "bulkImport";
