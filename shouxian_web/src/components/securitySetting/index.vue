		...
		<el-scrollbar>
			<el-collapse v-model="activeName" accordion @change="open">
					...
					<div class="content content2">
						<el-form label-width="120px" :model="pwForm">
							...
							<el-form-item label="新密码：">
								<el-input v-model="pwForm.newPassword" show-password @blur="verifyPwd('pwd')">
								</el-input>
							</el-form-item>
							<el-form-item label="确认新密码：">
								<el-input v-model="pwForm.newRePassword" show-password @blur="verifyPwd('confirmPwd')">
								</el-input>
							...
							<div class="passwordTip" :style="{color:pwdStyle?'':'#fc2449'}">
				...
				<el-collapse-item name="3">
					<template slot="title">
						<div class="tipTitle">绑定手机号 {{telphone}}</div>
					...
					<div class="content content3">
						<el-form v-if="telphone" label-width="120px" :model="phoneForm">
							<el-form-item label="旧手机号：">
								<el-input v-model="phoneForm.oldPhone"></el-input>
							</el-form-item>
							<el-form-item label="手机验证码：">
								<el-input v-model="phoneForm.oldCode"></el-input>
								<el-button type="warning" @click="getCode" plain>
									{{timer == 0 ||timer == -1? '发送验证码':timer +'s'}}
								</el-button>
								<span v-if="oldCodeWrong" class="codeWrong">验证码错误</span>
							</el-form-item>
						...
						<el-form v-else label-width="120px" :model="phoneForm">
							...
							<el-form-item label="手机验证码：">
								...
								<span v-if="newCodeWrong" class="codeWrong">验证码错误</span>
							</el-form-item>
							<el-form-item>
...
export default {
	...
	methods: {
		...
		async modifyPasswd() {
			...
			var result = await ajax.modifyPasswd(
				this.getTelphone,
				newPassword,
				password
			);
			...
			if (type == "pwd") {
				if (
					...
					if (this.pwForm.newRePassword.trim()) {
						if (
							this.pwForm.newPassword.trim() !=
							this.pwForm.newRePassword.trim()
						) {
							this.pwdBoth = false;
						} else {
							this.pwdBoth = true;
