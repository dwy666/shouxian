  ...
  <div id="jx-login">
    ...
    <div class="login-form">
      ...
      <div class="codeForm" v-if="active == 0">
        ...
        <div :style="{background:/^1[3456789]\d{9}$/.test(telphone.trim())?'#f29716':'#C1CBD7'}" class="getCode">获取验证码</div>
        <div v-if="isCode" class="codeTip">验证码错误</div>
        <el-checkbox v-model="checked">自动登录</el-checkbox>
        <div :style="{background:/^1[3456789]\d{9}$/.test(telphone.trim())&& /\d{6}$/.test(code.trim())?'#f29716':'#C1CBD7'}" class="loginBtn" @click="loginByVerify">登录</div>
      </div>
      <div class="passwdForm" v-else>
        ...
        <div :style="{background:/^1[3456789]\d{9}$/.test(telphone.trim())&& /\w{8,}$/.test(password.trim())?'#f29716':'#C1CBD7'}" class="loginBtn" @click="loginByPasswd">登录</div>
    ...
    methods: {
      ...
      async loginByPasswd() {
        if (
          ...
          var result = await ajax.loginByPasswd(this.telphone, this.password);
          if (result.code == "1005") {
